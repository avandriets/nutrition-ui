...
export class StatisticsApiService {
  ...
  getGoalForDate(accountId: number, userId: number, date: string): Observable<GoalTimelineResponse> {
  ...
  getNutritionAverage(accountId: number, userId: number, dateFrom: string, dateTo: string, includeEmptyDays: boolean): Observable<NutritionAverage> {
    const params = this.periodParams(dateFrom, dateTo, includeEmptyDays);
    return this.http.get<NutritionAverage>(`/api/accounts/${accountId}/users/${userId}/statistics/nutrition/average`, {
      params,
    });
  }
  ...
  getNutritionTimeline(
    ...
    const params = this.periodParams(dateFrom, dateTo, includeEmptyDays).set('granularity', granularity);
    return this.http.get<NutritionTimelineResponse>(`/api/accounts/${accountId}/users/${userId}/statistics/nutrition/timeline`, { params });
  }

  private periodParams(dateFrom: string, dateTo: string, includeEmptyDays: boolean): HttpParams {
    return new HttpParams().set('date_from', dateFrom).set('date_to', dateTo).set('include_empty_days', includeEmptyDays);
  }
}
