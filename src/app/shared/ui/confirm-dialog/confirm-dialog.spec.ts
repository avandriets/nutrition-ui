...
describe('UIConfirmDialogComponent', () => {
  it('renders configured content and danger state', async () => {
    const data: UIConfirmDialogData = {
      ...
      message: [{ text: 'Яблоко', emphasis: true }, { text: ' будет удалено.' }],
      confirmText: 'Удалить',
      tone: 'danger',
    ...
    expect(element.querySelector('h2')?.textContent).toContain('Удалить продукт?');
    expect(element.querySelector('strong')?.textContent).toBe('Яблоко');
    expect(element.querySelector('.danger')?.textContent).toContain('Удалить');
  });
});
